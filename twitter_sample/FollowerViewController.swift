...
class FollowerViewController: UITableViewController, UITableViewDelegate, UITableViewDataSource {
    var serviceManager = TwitterServiceManager()
    
    override func viewDidLoad() {
        serviceManager.fetchFollowerList(serviceManager.lastFetchedProfile["screen_name"] as! String, completion: { (success, result) -> Void in
            self.tableView.reloadData()
        })
    }
    
    override func tableView(tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        return serviceManager.follower.count
    }
    ...
    override func tableView(tableView: UITableView, cellForRowAtIndexPath indexPath: NSIndexPath) -> UITableViewCell {
        ...
        let folloing = serviceManager.follower[indexPath.row] as! Dictionary<String,AnyObject>
